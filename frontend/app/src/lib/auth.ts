...
async function refreshAccessToken(token: JWT): Promise<JWT> {
  try {
    ...
    return {
      ...
      accessTokenExpires: Date.now() + EXPIRY_MIN * 60 * 1000, // EXPIRY_MIN Minutes from now
    };
  } catch (error) {
      ...
      async authorize(credentials) {
        ...
        if (res.ok && data) {
          ...
          return {
            ...
            accessTokenExpires: decoded_user.exp? decoded_user.exp * 1000 : Date.now() + EXPIRY_MIN * 60 * 1000,
          };
        }
